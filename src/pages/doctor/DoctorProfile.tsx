...
const DoctorProfile: React.FC = () => {
  ...
  const handleSubmit = async (values: DoctorProfileFormValues) => {
    ...
    try {
      ...
      const workingHours = values.workingHours.reduce(
        (acc, wh) => ({
          ...acc,
          [wh.day]: { start: wh.start, end: wh.end },
        }),
      ...
      await apiService.post<DoctorProfile>("/doctor-profiles", payload);
      setSuccessMessage("Doctor profile created successfully");
      dispatch(clearError());
      setTimeout(() => setSuccessMessage(null), 3000);
    } catch (err: unknown) {
      const errorMessage =
        err instanceof Error ? err.message : "Failed to create doctor profile";
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <Field
                    ...
                    disabled={loading}
                    placeholder="e.g., 60d5ec9d1a6d8236f4673dce"
                    aria-required="true"
                  />
                  <ErrorMessage
                    name="user"
                ...
                <FieldArray name="workingHours">
                  {({ push, remove }) => (
                    <div className="space-y-4">
                      {/* Dropdown to add new working hours */}
